...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Build/search a FAISS vector DB for IP opinions (fast multilingual MiniLM version)"
    )
...
def build_document_texts(records: List[Dict[str, str]]) -> List[str]:
    # Za hitrost in enostavnost embedamo samo naslov.
...
def validate_build(output_dir: str, expected_count: int) -> None:
...
def run_search(args: argparse.Namespace) -> None:
    ...
    model = load_model(args.model, args.cache_dir)
    qvec = encode_query(model, args.query).reshape(1, -1)
